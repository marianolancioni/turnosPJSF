...
class OficinaController extends AbstractController
{
    ...
    public function addTurnos(Request $request, Oficina $oficina, TurnoRepository $turnoRepository, LoggerInterface $logger, OficinaRepository $oficinaRepository, SessionInterface $session, int $id = 1): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Si el multiplicador llega con valores decimales, reemplaza la "," por el "." decimal
            $multiplicadorTurnos = str_replace (',', '.', $multiplicadorTurnos);
     ...
     * @IsGranted("IS_AUTHENTICATED_ANONYMOUSLY")
     */
    public function addTurnosByOficinaID(Request $request, LoggerInterface $logger, int $oficinaIdDesde, int $oficinaIdHasta, $cantidadDias = 1): Response
    {
