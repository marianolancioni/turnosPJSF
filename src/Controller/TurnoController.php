...
class TurnoController extends AbstractController
{
    ...
    public function new2(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($_ENV['SISTEMA_ORALIDAD_CIVIL'] || $_ENV['SISTEMA_LABORATORIO_FORENSE']) {
        ...
        return $this->render('persona/new.html.twig', [
            'persona' => $persona,
            'organismo' => ($_ENV['SISTEMA_ORALIDAD_CIVIL'] || $_ENV['SISTEMA_LABORATORIO_FORENSE'] ? ($request->cookies->get('organismo') ? $request->cookies->get('organismo') : 0) : 0),
            'form' => $form->createView(),
        ]);
    }
    ...
    public function new3(Request $request): Response
    {
        ...
        $form = $this->createForm(Turno3Type::class, $turno, ['filtrarOficinas' => $this->_session->get('filtroOficinas')]);
    ...
    public function new5(Request $request, TurnosDiariosRepository $turnosDiariosRepository, ConfigRepository $configRepository): Response
    {
        ...
        // Busco la localidad del Organismo vinculado (caso de Oralidad Civil y de Laboratorio Forense)
        $localidadOrganismo = '';
        if ($_ENV['SISTEMA_ORALIDAD_CIVIL'] || $_ENV['SISTEMA_LABORATORIO_FORENSE']) {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!$turnoActualizar) {
                ...
                // Si se trata de agenda de Oralidad y de Laboratorio Forense asocio el Organismo a la Persona para
                // que se persista Persona en la BD sin problemas
                if ($_ENV['SISTEMA_ORALIDAD_CIVIL'] || $_ENV['SISTEMA_LABORATORIO_FORENSE']) {
                    $persona->setOrganismo($organismo);
                }

                // Si se trata de agenda de Oralidad asocio atributos específicos
                if ($_ENV['SISTEMA_ORALIDAD_CIVIL']) {
        ...
        if (!is_null($turno) && !is_null($turno->getPersona())) {
            ...
            if ($turno->getPersona()->getEmail()) {
                try {
                    ...
                    if ($_ENV['SISTEMA_LABORATORIO_FORENSE']) {
                        $mailTemplate = 'turno/email_confirmacion_turno_laboratorio_forense.html.twig';
                    }
    ...
    /**
     * Invoca al Turnero con una URL de filtro y un parámetro que es la clave del
     * conjunto clave/valor del Archivo Config. Si la clave se encuentra definida
     * asigna a nivel de session los valores contenidos en dicha clave que representan
     * una lista de ID de Oficinas a Filtrar en el Turnero
     * 
     * @Route("/TurnosWeb/filtrar/{filtro}", name="filter", methods={"GET","POST"})
     * 
     */
    public function filtrarOficinas(ConfigRepository $configRepository, $filtro): Response
    {
        // Busca clave de configuración con lista de oficinas a filtrar
        if ($config = $configRepository->findByClave($filtro)) {
            $filtroOficinas = $config->getValor();  // Obtienen la lista de Oficinas a filtrar
            $this->_session->set('filtroOficinas', $filtroOficinas);                // Guarda en Session la lista de Oficinas
        } 

        return $this->redirectToRoute('turno_new2');                                // Redirige al primer paso de Obtención de Turnos
    }    
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                if (isset($request->request->get('turno_rechazar')['enviarMail']) && $turno->getPersona() && $turno->getPersona()->getEmail()) {
                    ...
                    try {
                        ...
                        if ($_ENV['SISTEMA_LABORATORIO_FORENSE']) {
                            $mailTemplate = 'turno/email_rechazado_laboratorio_forense.html.twig';
                        }                        
     ...
     * Si viene a nivel de Session las Oficinas ya predefinidas, solo obtiene las localidades correspondientes a la mismas
     * 
    ...
    {
        if ($this->_session->get('filtroOficinas')) {
            $localidades = $this->_oficinaRepository->findByLocalidadesByCircunscripcionByOficinas($circunscripcion_id, $this->_session->get('filtroOficinas'));
        }
        else {
     ...
     * Si viene a nivel de Session las Oficinas ya predefinidas, solo obtiene las Oficinas correspondientes a la mismas
     * 
    ...
    {
        if ($this->_session->get('filtroOficinas')) {
            $oficinas = $this->_oficinaRepository->findOficinasHabilitadasByLocalidadByList($localidad_id, $this->_session->get('filtroOficinas'));
        }
        else {
    ...
    {
        if ($this->_session->get('filtroOficinas')) {
            $oficinas = $this->_oficinaRepository->findOficinasByLocalidadWithTelefonoByList($localidad_id, $this->_session->get('filtroOficinas'));
        }
        else {
     ...
     * 
     * Se la busca por el ID del Organismo Asociado. Por lo tanto sólo es útil en modo de operación SISTEMA_ORALIDAD_CIVIL o SISTEMA_LABORATORIO_FORENSE
     * 
     ...
     * 
     * @Route("/TurnosWeb/diasOcupadosOficina/{oficina_id}/{organismoId}", name="diasOcupadosOficina", requirements = {"oficina_id" = "\d+"}, methods={"GET", "POST"})
     * 
     * @param $oficina_id   Turnero sobre el cual se está obteniendo el turno
     * @param $organismoId  Organismo que solicita el turno (argumento opcional y aplicable al Turnero de Bioquímica Forense)
     * 
     */
    public function diasOcupadosByOficina($oficina_id, $organismoId = null)
    {       
        if ($oficina_id != null) {
            ...
            if (!is_null($organismoId)) { // Este argumento sólo es recibido por petición Ajax en el contexto del Turnero de Laboratorio de Bioquímica Forense
                $rangoHorario = $this->rangoHorariosByOficina($organismoId);
                $hDesde = $rangoHorario['desde'] ?? '00:00';
                $hHasta = $rangoHorario['hasta'] ?? '23:59';               

                $desde = (new \DateTime)->createFromFormat('d/m/Y H:i:s', $primerDiaDisponible . $hDesde . ':00');
                $hasta = (new \DateTime)->createFromFormat('d/m/Y H:i:s', $ultimoDiaDisponible . $hHasta . ':59');
            }
            ...
            while (true && ($desde && $hasta)) {
                // Establece horario máximo de búsqueda. Se busca desde las 0hs hasta las 23:59, día a día
                if (is_null($organismoId)) {
                    $horaHasta = (new \DateTime)->createFromFormat('d/m/Y H:i:s', $desde->format('d/m/Y') . ' 23:59:59');
                } else {
                    $horaHasta = (new \DateTime)->createFromFormat('d/m/Y H:i:s', $desde->format('d/m/Y') . ' ' . $hHasta . ':59');
                }
     ...
     * 
     * @Route("/TurnosWeb/horariosDisponiblesOficinaFecha/{oficina_id}/{fecha}/{organismoId}", name="horarisDisponibles", methods={"POST"})
     * @param $oficina_id   Turnero sobre el cual se está obteniendo el turno
     * @param $fecha        Fecha seleccionada para el turno
     * @param $organismoId  Organismo que solicita el turno (argumento opcional y aplicable al Turnero de Bioquímica Forense)
     */
    public function horariosDisponiblesByOficinaByFecha($oficina_id, $fecha, $organismoId = null)
    {
        $desde = null;
        $hasta = null;
        if (!is_null($organismoId)) { // Este argumento sólo es recibido por petición Ajax en el contexto del Turnero de Laboratorio de Bioquímica Forense
            $rangoHorario = $this->rangoHorariosByOficina($organismoId);
            $desde = $rangoHorario['desde'];
            $hasta = $rangoHorario['hasta'];
        }

        $horariosDisponibles = $this->_turnoRepository->findHorariosDisponiblesByOficinaByFecha($oficina_id, $fecha, $desde, $hasta);
        return new JsonResponse($horariosDisponibles);
    }

    /**
     * Devuelve rango horario de la Oficina que solicita el turno
     * (sólo válido para el turnero de Laboratorio de Bioquímica Forense)
     * @param $organismoId ID del Organismo que está solicitando el Turno
     */
    public function rangoHorariosByOficina($organismoId)
    {
        //Grupo de Oficinas entre las 7:45hs a 8:00hs (Defensorías Generales Civiles)
        if (str_contains('101079, 101080, 101081, 101164, 101168, 101267, 101268, 101269, 101270, 101371', $organismoId)) {
            return ['desde' => '07:45', 'hasta' => '08:00'];
        }

        //Grupo de Oficinas entre las 8:15hs a 10:00hs (Tribunales Colegiados de Familia N° 2, 3 y 5)
        if (str_contains('101128, 101349, 101350', $organismoId)) {
            return ['desde' => '08:15', 'hasta' => '10:00'];
        }

        //Grupo de Oficinas entre las 8:15hs a 10:00hs (Juzgados. de 1ra Inst. de Dist. en lo C. C. y Laboral de San Justo y San Javier)
        if (str_contains('111002, 112002', $organismoId)) {
            return ['desde' => '08:15', 'hasta' => '10:00'];
        }

        //Grupo de Oficinas entre las 10:15hs a 11:00hs (Juzg. de 1ra Inst. de Distrito de Santa Fe y MPA)
        if (str_contains('101011, 101012, 101013, 101014, 101015, 101016, 101017, 101018, 101019, 101020, 101110, 501007', $organismoId)) {
            return ['desde' => '10:15', 'hasta' => '11:00'];
        }

        // Si el organismo no se encontró retorna nulos lo que provocará en una rango completo de horarios para la selección del turno
    ...
    public function ocupacionAgenda(Request $request)
    {
        ...
        if ($this->isGranted('ROLE_ADMIN') || $this->isGranted('ROLE_AUDITORIA_GESTION')) {
            ...
            $cantidadTurnosExistentes = $this->_turnoRepository->findCantidadTurnosExistentes($filtroOficina, !is_null($this->getUser()->getCircunscripcion()) ? $this->getUser()->getCircunscripcion()->getId() : null);
            if ($cantidadTurnosExistentes > 0)
            ...
            if ($this->isGranted('ROLE_USER')) {
                ...
                $cantidadTurnosExistentes = $this->_turnoRepository->findCantidadTurnosExistentes($oficinaUsuario, !is_null($this->getUser()->getCircunscripcion()) ? $this->getUser()->getCircunscripcion()->getId() : null);
                if ($cantidadTurnosExistentes > 0)
