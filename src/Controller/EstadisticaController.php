...
class EstadisticaController extends AbstractController
{
    ...
    /**
     * @Route("/estadistica/informeFDMensual", name="informe_fd_mensual", methods={"GET", "POST"})
     * @IsGranted("ROLE_AUDITORIA_GESTION")
     */
    public function informeFDMensual(OficinaRepository $oficinaRepository): Response
    {

        // Propone fechas (dese el comienzo del turnero hasta el día actual)
        $desde = new \DateTime('05/11/2020');
        $hasta = new \DateTime(date("Y-m-d") . " 00:00:00");

        $oficinas = $oficinaRepository->findOficinasFD();

        return $this->render('estadistica/informeFDMensual.html.twig', [
            'desde' => $desde->format('d/m/Y'),
            'hasta' => $hasta->format('d/m/Y'),
            'oficinas' => $oficinas,
        ]);
    ...
    /**
     * @Route("/estadistica/showInformeFDMensual", name="show_InformeFDMensual", methods={"GET", "POST"})
     * @IsGranted("ROLE_AUDITORIA_GESTION")
     */
    public function showInformeFDMensual(Request $request, TurnoRepository $turnoRepository, OficinaRepository $oficinaRepository, CircunscripcionRepository $circunscripcionRepository, LoggerInterface $logger): Response
    {
        ...
        $result = $turnoRepository->obtenerEstadisticaFD($desde, $hasta, $oficinaId);

        // Audito la acción
        $logger->info('Se emite informe de Estadística de FD Mensual', [
            'Usuario' => $this->getUser()->getUsuario()
        ]);

        return $this->render('estadistica/showInformeFDMensual.html.twig', [
            'desde' => substr($desde, 0, 10),
            'hasta' => substr($hasta, 0, 10),
            'result' => $result,
        ]);
