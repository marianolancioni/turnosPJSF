...
class Turno3Type extends AbstractType
{
    ...
    {
        // Si viene predefinida a nivel de Session una lista de Oficinas, muestra únicamente las
        // Circunscripciones que contienen a esas Oficinas. En caso contrario muestra todas.
        if ($options['filtrarOficinas']) {
            $builder
                ->add('circunscripcion', EntityType::class, [
                    'class' => 'App\Entity\Circunscripcion',
                    'query_builder' => function (CircunscripcionRepository $er) use ($options) {
                        return $er->createQueryBuilder('c')                                    
                                    ->innerJoin('c.localidad', 'l')
                                    ->innerJoin('l.oficinas', 'o')
                                    ->where('o in (' . $options['filtrarOficinas'] .')');
                    },
        ...
        if ($_ENV['SISTEMA_LABORATORIO_FORENSE']) {
            $builder->add('motivo',  TextType::class, ['required' => true, 'help'   => 'Ej.: 21-22523087-8, Ferreyra Mariano, DNI 29019381', 'attr' => array('maxlength' => '255')]);
        }
