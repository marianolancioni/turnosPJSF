...
class OficinaRepository extends ServiceEntityRepository
{
    ...
    public function findOficinasHabilitadasByLocalidadByList($localidad_id, $filtroOficinas)
    {
        return $this->createQueryBuilder('o')
            ->select('o.id, o.oficina as oficina')
            ->andWhere('o.habilitada = true and o.localidad = :val')
            ->andWhere('o in (' . $filtroOficinas . ')')
            ->setParameter('val', $localidad_id)
            ->orderBy('oficina')
            ->getQuery()
            ->getArrayResult();;
    ...
    public function findOficinasByLocalidadWithTelefonoByList($localidad_id, $filtroOficinas)
    {
        return $this->createQueryBuilder('o')
            ->select('o.id, o.oficina as oficina, o.telefono, o.habilitada')
            ->andWhere('o.localidad = :val')
    ...
    public function findByLocalidadesByCircunscripcionByOficinas($circunscripcion_id, $filtroOficinas)
    {
        return $this->createQueryBuilder('o')
            ->select('l.id, l.localidad as localidad')
            ->innerJoin('o.localidad', 'l', 'WITH', 'l.circunscripcion in (:val)')
    ...
    // Obtienen Oficinas de Firma Digital (aquellas que contienen en su nombre la palabra "Token")
    public function findOficinasFD()
    {
        return $this->createQueryBuilder('o')
            ->select('o.id, o.oficina as oficina, l.localidad')
            ->innerJoin('o.localidad', 'l')
            ->andWhere("toUpper(o.oficina) like '%TOKEN%'")
            ->orderBy('l.localidad')
            ->getQuery()
            ->getResult();
    }
