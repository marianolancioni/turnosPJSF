...
class TurnoRepository extends ServiceEntityRepository
{
    ...
    public function findHorariosDisponiblesByOficinaByFecha($oficina_id, $fecha, $horaDesde = null, $horaHasta = null)
    {

        // Evalúo filtro de rango horario y si es necesario armo filtro en función a ello
        $filtroRangoHorario = '';
        if (!is_null($horaDesde) && !is_null($horaHasta)) {
            $filtroRangoHorario = "AND fecha_hora >= '" . $fecha . " " . $horaDesde . "'";
            $filtroRangoHorario .= " AND fecha_hora <= '" . $fecha . " " . $horaHasta . "'";
        }
        // Obtengo días (futuros) con turnos disponibles para una oficina en particular
        $sql = "SELECT to_char(fecha_hora, 'HH24:MI') as hora FROM turno WHERE persona_id is null AND oficina_id = :oficina_id AND to_char(date(fecha_hora), 'dd-mm-yyyy') = :fecha and fecha_hora > now() $filtroRangoHorario order by fecha_hora";
    ...
    /**
     * Obtiene datos para el informe de Estadísticas de Firma Digital
     */
    public function obtenerEstadisticaFD($desde, $hasta, $oficinaId)
    {
        ...
        // Agrupa Turnos de FD mensualmente 
        $sql = "SELECT Num_Circunscripción, Circunscripción, localidad, to_char(mes_firma, '09') || '/' || to_char(año_firma,'9999') Entrega_firma,
                    to_char(mes_vencimiento,'09') || '/' || to_char(año_vencimiento,'9999') Vencimiento_firma, firmas_entregadas
                FROM (
                    SELECT c.id AS Num_Circunscripción, c.circunscripcion AS Circunscripción, l.localidad localidad, extract(month FROM t.fecha_hora) as mes_firma,
                        extract(year FROM t.fecha_hora) AS año_firma, extract(month FROM t.fecha_hora) AS mes_vencimiento,
                        extract(year FROM t.fecha_hora) + 2 AS año_vencimiento, count(*) AS firmas_entregadas
                    FROM turno t, oficina o, localidad l, circunscripcion c
                    WHERE t.oficina_id = o.id
                        AND o.localidad_id = l.id
                        AND o.oficina like '%Token%'
                        AND t.persona_id is not null
                        AND c.id = l.circunscripcion_id
                        AND t.fecha_hora >= '$fechaDesde'
                        AND t.fecha_hora <= '$fechaHasta'
                        AND t.estado = 2
                        $filtroOficina
                    GROUP BY Num_Circunscripción, localidad, mes_firma, año_firma
                    ORDER BY Num_Circunscripción, localidad, año_firma asc, mes_firma asc) temp";
