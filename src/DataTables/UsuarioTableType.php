...
class UsuarioTableType extends AbstractController implements DataTableTypeInterface
{
    ...
    public function configure(DataTable $dataTable, array $options)
    {
        ...
        $dataTable->add('fecha_alta', DateTimeColumn::class, ['label' => 'Alta', 'format' => 'd/m/y', 'className' => 'text-center', 'searchable' => true, 'operator' => 'LIKE', 'leftExpr' => "toChar(u.fecha_alta, 'DD/MM/YY')", 'rightExpr' => function ($value) {
            return '%' . $value . '%';
        }]);
        $dataTable->add('fecha_baja', DateTimeColumn::class, ['label' => 'Baja', 'format' => 'd/m/y', 'className' => 'text-center', 'searchable' => true, 'operator' => 'LIKE', 'leftExpr' => "toChar(u.fecha_baja, 'DD/MM/YY')", 'rightExpr' => function ($value) {
            return '%' . $value . '%';
        }]);
        $dataTable->add('ultimo_acceso', DateTimeColumn::class, ['label' => 'Último Acceso', 'format' => 'd/m/y', 'className' => 'text-center', 'searchable' => true, 'operator' => 'LIKE', 'leftExpr' => "toChar(u.ultimo_acceso, 'DD/MM/YY')", 'rightExpr' => function ($value) {
            return '%' . $value . '%';
        }]);
        ...
        if ($this->isGranted(('ROLE_EDITOR'))) {
            $dataTable->add('acciones', TextColumn::class, ['label' => 'Acciones', 'className' => 'text-center', 'render' => function ($value, $context) {
                $acciones  = '&nbsp;&nbsp;<a href="' . $this->generateUrl('usuario_show', ['id' => $context->getId()]) . '" title="Ver"><i class="fas fa-eye"></i></a>';
                $acciones .= '&nbsp;&nbsp;<a href="' . $this->generateUrl('usuario_edit', ['id' => $context->getId()]) . '" title="Editar"><i class="fas fa-pen"></i></a>';
                if ($this->isGranted(('ROLE_ADMIN'))) {
                    $acciones .= '&nbsp;&nbsp;<a href="' . $this->generateUrl('usuario_index', ['_switch_user' => $context->getUsername()]) . '" title="Adquirir privilegios de ' . $context->getUsername() . '"><i class="fas fa-user-secret"></i></a>';
                }
